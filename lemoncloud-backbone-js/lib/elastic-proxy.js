...
module.exports = function(p, value) {
  ...
  p(value, val);
  var event = constants.env("ES_ENDPOINT");
  ...
  var makeScrollBarDecorator = function() {
    if (!event) {
      throw new Error("env:ES_ENDPOINT is required!");
    }
    var key = "X" + value;
    var result = p(key);
